...
bool DBSCAN::Init(vector<Point2i> points, double radius, int minPTs)
{
	...
	for (size_t i = 0; i < length; i++)
	{
		...
		point.SetDpId(i);                    //将数据点对象ID设置为i
		point.SetVisited(false);            //数据点对象isVisited设置为false
		point.SetClusterId(-1);            //设置默认簇ID为-1
		dadaSets.push_back(point);            //将对象压入数据集合容器
	}
	dataNum = length;						//设置数据对象集合大小为i
	for (unsigned long i = 0; i<dataNum; i++)
	{
		SetArrivalPoints(dadaSets[i]);            //计算数据点领域内对象
	}
